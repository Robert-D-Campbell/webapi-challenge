  ...
  if (!req.params.id) {
    ...
    Action.update(actionsId, changes)
      .then(actions => {
        res.status(200).json({
          message:
            "Successfully Edited the specified action for the specified project"
        });
      })
      .catch(err => {
        console.log("error on PUT /api/projects/:id/actions/:actions_id", err);
        res.status(500).json({
          errorMessage: "error editing the actions array to the project"
        });
      });
  }
});
...
router.delete("/:id/actions/:actions_id", (req, res) => {
  ...
  Action.remove(actionsId)
    .then(removed => {
      res.status(200).json({
        message:
          "successfully removed Action with the Specified Action ID and Project ID"
      });
    })
    .catch(error => {
      console.log(error);
